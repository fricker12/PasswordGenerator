...
#def generate_password_from_template_test(template, character_set):
...
#            #password += ''.join(random.sample(character_set, len(character_set)))
...
def generate_password_from_template(template, character_set):
    ...
    while i < len(template):
        if template[i] == '\\':
            ...
            password +=random.choice(character_set)
            #if character_set:
            #    last_char = password[-1] if password else ''
            #    new_char = random.choice([c for c in character_set if c != last_char])
            #    password += new_char
            i += 1
    return password
